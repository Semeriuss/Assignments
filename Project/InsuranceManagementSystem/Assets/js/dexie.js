...
(function(global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined'
		...
		: typeof define === 'function' && define.amd ? define(factory) : (global.Dexie = factory());
})(this, function() {
	'use strict';
	...
	var getValueOf = function(val, type) {
		return type === 'Array'
			...
			: type === 'ArrayBuffer'
				? '' + new Uint8Array(val)
				: type === 'Date' ? val.getTime() : ArrayBuffer.isView(val) ? '' + new Uint8Array(val.buffer) : val;
	};
	...
	var Table = (function() {
		...
		Table.prototype.where = function(indexOrCrit) {
			...
			return idx
				...
				: compoundIndex ? this.filter(filterFunction) : this.where(keyPaths).equals('');
		};
			...
			return this._write(function(trans) {
				...
				return _this.clone().primaryKeys().then(function(keys$$1) {
					var nextChunk = function(offset) {
						...
						return coreTable
							.getMany({ trans: trans, keys: keys$$1.slice(offset, offset + count) })
								...
								return Promise.resolve(
									...
									.then(function(res) {
										return (
											putValues.length > 0 &&
											coreTable
												.mutate({ trans: trans, type: 'put', keys: putKeys, values: putValues })
												.then(function(res) {
									...
									.then(function() {
										return (
											deleteKeys.length > 0 &&
											coreTable
												.mutate({ trans: trans, type: 'delete', keys: deleteKeys })
												.then(function(res) {
	...
	function createCollectionConstructor(db) {
		return makeClassConstructor(Collection.prototype, function Collection$$1(whereClause, keyRangeGenerator) {
			...
			this._ctx = {
				...
				isPrimKey:
					!whereCtx.index || (table.schema.primKey.keyPath && whereCtx.index === table.schema.primKey.name),
				range: keyRange,
				keysOnly: false,
				dir: 'next',
				unique: '',
	...
	function createDBCore(db, indexedDB, IdbKeyRange, tmpTrans) {
		...
		function makeIDBKeyRange(range) {
			...
			var idbRange =
				lower === undefined
					? upper === undefined ? null : IdbKeyRange.upperBound(upper, !!upperOpen)
					: upper === undefined
						...
						function createVirtualCursor(cursor) {
							function _continue(key) {
								key != null
									...
									: req.unique
										? cursor.continue(
												pad(cursor.key, req.reverse ? down.MIN_KEY : down.MAX_KEY, keyTail)
											)
										: cursor.continue();
							}
							...
							function addPutOrDelete(req) {
								...
								req =
									req.type === 'add' || req.type === 'put'
										? __assign(__assign({}, req), { keys: keys$$1, wantResults: true })
										: __assign({}, req);
		...
		function Dexie(name, options) {
			...
			this.on('blocked', function(ev) {
				if (!ev.newVersion || ev.newVersion < ev.oldVersion)
					...
					console.warn(
						"Upgrade '" + _this.name + "' blocked by other connection holding version " + ev.oldVersion / 10
					);
			});
